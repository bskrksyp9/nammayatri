...
view push config = 
  linearLayout 
    ...
    , if config.specialZoneTag == Maybe.Nothing then rideActionView push config else rideActionViewWithZone push config
    ]
...
rideActionViewWithZone push config =
  ...
  [ width MATCH_PARENT
  , height WRAP_CONTENT
  , background (getZoneTagConfig config.specialZoneTag).backgroundColor
  , cornerRadii $ Corners 25.0 true true false false
  , orientation VERTICAL
  , padding $ PaddingTop 5
  , gravity CENTER
  , visibility if config.specialZoneTag == Maybe.Nothing then GONE else VISIBLE
  ][ linearLayout
      [ width MATCH_PARENT
      , height WRAP_CONTENT
      , gravity CENTER
          ...
          [ width $ V 18
          , height $ V 18
          , imageUrl (getZoneTagConfig config.specialZoneTag).imageUrl
          ]
        , textView 
          [ width WRAP_CONTENT
          , height MATCH_PARENT
          , text (getZoneTagConfig config.specialZoneTag).text
...
rideActionView push config =
  ...
  , gravity CENTER
  , stroke $ "1," <> Color.grey800
  , visibility if config.specialZoneTag == Maybe.Nothing then VISIBLE else GONE
  ][  rideActionDataView push config
    , linearLayout
      [ width MATCH_PARENT
      , height $ V 1
